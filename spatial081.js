...
(function(){
  ...
  function activeTeamPlayers(team,c){
    try{return [...new Set(Object.values(c?.teams?.[team]||{}).map(a=>a?.pid).filter(Boolean))]}catch(e){return[]}
  }
  ...
  function showOpponentRecoveryPicker(lostEvent){
    ...
    document.getElementById('sp081PitchWrap').classList.add('hidden');document.getElementById('sp081RecoverPick').classList.add('show');document.getElementById('sp081Cancel').textContent='Nessun recupero';document.getElementById('sp081Title').textContent='❌ Palla persa registrata';document.getElementById('sp081Meta').textContent=`LIVE · Squadra ${lostEvent.team} → possibile recupero Squadra ${opp}`;document.getElementById('sp081RecoverHelp').textContent='Seleziona solo un giocatore attualmente in campo. Il recupero avrà stesso tempo e stesse coordinate della palla persa.';
    choices.innerHTML=ids.map(pid=>`<button class="sp081choice" data-sp081recover="${pid}">🔄 ${playerName(pid)}</button>`).join('')+`<button class="sp081choice sp081none" data-sp081recover="">Nessun recupero avversario</button>`;
    choices.querySelectorAll('[data-sp081recover]').forEach(b=>b.onclick=()=>finishRecoveryPair(b.dataset.sp081recover,opp));
  }
  function finishRecoveryPair(pid,opp){
    const c=C();if(!lastLostEvent){close();return}if(pid){const pairId='pair'+Date.now()+Math.random();lastLostEvent.paired_event_id=pairId;const r={id:Date.now()+Math.random(),pid,type:'recover',t:lastLostEvent.t,phase:lastLostEvent.phase||c.phase,team:opp,void:false,x_m:lastLostEvent.x_m,y_m:lastLostEvent.y_m,x_pct:lastLostEvent.x_pct,y_pct:lastLostEvent.y_pct,field_length_m:FIELD_L,field_width_m:FIELD_W,field_third:lastLostEvent.field_third,field_lane:lastLostEvent.field_lane,spatial:true,attack_direction:'x_positive',paired_event_id:pairId,paired_with_pid:lastLostEvent.pid,paired_source:'live_lost'};lastLostEvent.paired_with_pid=pid;lastLostEvent.paired_source='live_lost';c.events=c.events||[];c.events.push(r);save();try{toast('Palla persa + recupero registrati')}catch(e){}}
  ...
  function choose(ev){
    ...
    addEvent(p.pid,p.type);const c=C(),events=c.events||[];if(events.length<=before){close();return}let e=null;for(let i=events.length-1;i>=0;i--){const q=events[i];if(!q.void&&String(q.pid)===String(p.pid)&&q.type===p.type){e=q;break}}if(!e){close();return}attachSpatial(e,p,x,y,xp,yp);save();
    const dot=document.getElementById('sp081Dot');dot.hidden=false;dot.style.left=(xp*100)+'%';dot.style.top=(yp*100)+'%';document.getElementById('sp081Coords').textContent=`X ${x.toFixed(1)} m · Y ${y.toFixed(1)} m`;
    if(!isGame(c)&&p.type==='lost'){setTimeout(()=>showOpponentRecoveryPicker(e),80);return}setTimeout(()=>{close();try{renderLive()}catch(err){}},110);
  }
