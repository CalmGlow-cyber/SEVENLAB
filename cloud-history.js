...
(function(){
  ...
  async function syncEvents(s){
    await req('events?sessione_id=eq.'+encodeURIComponent(s.id),{method:'DELETE'});
    const ev=(s.events||[]).map(e=>({id:String(e.id),sessione_id:s.id,tipo:e.type||'evento',giocatore_id:e.pid?String(e.pid):null,team:e.team||null,secondi:Math.max(0,Math.floor(e.t||0)),phase:e.phase||null,payload:e}));
    if(ev.length)await req('events',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(ev)});
  }
  async function upsert(s){
    const tipo=s.tipo==='partita'?'partita':'allenamento';
    await req('sessions?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({id:s.id,tipo,avversario:tipo==='partita'?(s.avversario||null):null,gol_fatti:s.score?.A||0,gol_subiti:s.score?.B||0,session_data:s,updated_at:new Date().toISOString()})});
    await syncEvents(s);
  }
