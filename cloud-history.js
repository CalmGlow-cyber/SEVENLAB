// SevenLab Cloud history sync 0.6.5. Official sessions sync to Supabase; Test Mode stays local only.
(function(){
  ...
  async function syncEvents(s){if(s?.testMode)return;await req('events?sessione_id=eq.'+encodeURIComponent(s.id),{method:'DELETE'});const ev=(s.events||[]).map(e=>({id:String(e.id),sessione_id:s.id,tipo:e.type||'evento',giocatore_id:e.pid?String(e.pid):null,team:e.team||null,secondi:Math.max(0,Math.floor(e.t||0)),phase:e.phase||null,payload:e}));if(ev.length)await req('events',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(ev)})}
  async function upsert(s){if(s?.testMode)return;const tipo=s.tipo==='partita'?'partita':'allenamento';await req('sessions?on_conflict=id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({id:s.id,tipo,avversario:tipo==='partita'?(s.avversario||null):null,gol_fatti:s.score?.A||0,gol_subiti:s.score?.B||0,session_data:s,updated_at:new Date().toISOString()})});await syncEvents(s)}
  async function remove(id){return req('sessions?id=eq.'+encodeURIComponent(id),{method:'DELETE'})}
  async function pull(){if(!window.SevenLabAuth?.token)return;if(syncing)return;syncing=true;try{const rows=await req('sessions?select=id,tipo,avversario,gol_fatti,gol_subiti,session_data,created_at&order=created_at.desc');const officialLocal=(DB.history||[]).filter(s=>!s?.testMode);if(!initialized&&!rows.length&&officialLocal.length){for(const s of officialLocal)await upsert(s);initialized=true;toast('Storico caricato nel cloud ☁️');return}initialized=true;DB.history=(rows||[]).map(r=>{const s=r.session_data||{};s.tipo=r.tipo||s.tipo||'allenamento';s.testMode=false;if(r.tipo==='partita'){s.avversario=r.avversario||s.avversario||'';s.score=s.score||{A:0,B:0};s.score.A=r.gol_fatti??s.score.A??0;s.score.B=r.gol_subiti??s.score.B??0}s.id=s.id||r.id;return s}).filter(Boolean);cache();renderHistory()}catch(e){console.error('SevenLab history pull',e);if(e.message!=='AUTH_REQUIRED')toast('Storico cloud non raggiungibile')}finally{syncing=false}}
  const stableArchive=archive;archive=function(){const before=new Set(DB.history.map(s=>s.id));stableArchive();const s=DB.history.find(x=>!before.has(x.id));if(s?.testMode)return;if(s&&window.SevenLabAuth?.token)upsert(s).then(()=>pull()).catch(e=>{console.error(e);toast('Sessione locale: cloud non raggiungibile')})};
...
// 0.6.5: team-profile.js and patch061.js remain loaded directly by index.html.
