// SevenLab Auth gate — whitelist + password recovery.
(function(){
  ...
  function saveSession(s){session=s;localStorage.setItem(storeKey,JSON.stringify(s))} function clearSession(){session=null;localStorage.removeItem(storeKey)}
  ...
  async function validate(){if(!session?.access_token)return false;let email=session.user?.email;if(!email)return false;let ok=await allowed(session.access_token,email);if(!ok&&await refresh())ok=await allowed(session.access_token,session.user?.email);if(!ok)clearSession();return ok}
  ...
  async function signup(email,password){if(email.trim().toLowerCase()!==ALLOWED_FIRST)throw new Error('Email non autorizzata.');const d=await post('signup',{email,password});if(d.access_token){saveSession(d);showApp();location.reload()}else showGate('Account creato. Ora puoi accedere dopo la conferma email.')}
  async function recover(email){if(!email)throw new Error('Inserisci prima la tua email.');const redirect=location.origin+location.pathname;await post('recover',{email},{'x-redirect-to':redirect});showGate('Ti ho inviato il link per scegliere una nuova password. Controlla anche Spam.')}
  function recoverySessionFromUrl(){const h=new URLSearchParams(location.hash.replace(/^#/,''));if(h.get('type')!=='recovery'||!h.get('access_token'))return null;return {access_token:h.get('access_token'),refresh_token:h.get('refresh_token')||'',user:{email:''}}}
  async function setNewPassword(password,token){if(password.length<6)throw new Error('Usa almeno 6 caratteri.');const r=await fetch(URL+'/auth/v1/user',{method:'PUT',headers:authHeaders(token),body:JSON.stringify({password})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.msg||d.message||d.error||'Impossibile aggiornare la password');history.replaceState(null,'',location.pathname);clearSession();showGate('Password aggiornata ✅ Ora accedi con la nuova password.')}
  ...
  window.addEventListener('DOMContentLoaded',async()=>{
    ...
    const rec=recoverySessionFromUrl();
    if(rec){showGate('Scegli qui sotto la nuova password, poi premi “Salva nuova password”.');signupBtn.style.display='none';loginBtn.textContent='Salva nuova password';forgot.style.display='none';loginBtn.onclick=async()=>{try{loginBtn.disabled=true;await setNewPassword(document.getElementById('authPassword').value,rec.access_token)}catch(e){showGate(e.message)}finally{loginBtn.disabled=false}};return}
