...
(function(){
  ...
  const baseGo=go;
  go=function(id){baseGo(id);if(id==='game')renderGame();if(id==='live'&&isGame()){$$('.nav button').forEach(x=>x.classList.toggle('active',x.dataset.go==='game'))}};

  const baseRenderLive=renderLive;
  renderLive=function(){
    baseRenderLive();const c=C();if(!isGame())return;
    const score=$('.scorebox');let names=$('.gameScoreNames');if(!names){names=document.createElement('div');names.className='gameScoreNames';score?.parentElement?.insertBefore(names,score)}names.innerHTML=`<span>SEVENLAB</span><span>${esc(c.avversario||'AVVERSARIO')}</span>`;
    $$('[data-score]').forEach(b=>b.style.display='none');$$('#livePlayers .liveplayer').forEach(card=>{const role=card.querySelector('.role');if(role)role.textContent='SevenLab'});
    $$('#livePlayers [data-ev$="|save"]').forEach(saveBtn=>{const grid=saveBtn.closest('.countergrid');if(!grid||grid.querySelector('.gameAgainst'))return;const btn=document.createElement('button');btn.className='counter counterbig gameAgainst';btn.innerHTML='<b>🥅 '+(c.score.B||0)+'</b><small>Gol subito</small>';btn.onclick=()=>{c.score.B=(c.score.B||0)+1;c.gol_subiti=c.score.B;c.events.push({id:Date.now()+Math.random(),pid:null,type:'goal_against',t:elapsed(),phase:c.phase,team:'B',void:false});save();renderLive();toast('Gol subito registrato')};grid.appendChild(btn)});
    const head=$('#live .card');if(head&&!head.querySelector('.gameLiveBadge')){const x=document.createElement('div');x.className='gameLiveBadge';x.textContent='🎮 PARTITA UFFICIALE';head.insertBefore(x,head.firstChild)}const meta=head?.querySelector('.gameMeta')||document.createElement('div');if(!meta.classList.contains('gameMeta'))meta.className='gameMeta';meta.innerHTML=`<span>vs ${esc(c.avversario||'')}</span><span>${c.teamModules?.A||c.module||''}</span>`;if(head&&!meta.parentElement)head.appendChild(meta);
  ...
  const baseCommitGoal=commitGoal;commitGoal=function(assistId){baseCommitGoal(assistId);if(isGame()){C().gol_fatti=C().score.A||0;save()}};

  archive=function(){let c=JSON.parse(JSON.stringify(C()));c.startedAt=null;c.activeSegment=null;c.tipo=sessionType(c);c.gol_fatti=c.score?.A||0;c.gol_subiti=c.score?.B||0;c.playerScores={};c.present.forEach(id=>c.playerScores[id]=playerScore(id,c));DB.history.unshift(c);DB.current=EC();save();renderPresence();renderHistory();go('storico');toast(c.tipo===GAME_TYPE?'Partita salvata':'Allenamento salvato')};
  ...
  function withHistory(type,fn){const old=DB.history;DB.history=old.filter(s=>sessionType(s)===type);try{return fn()}finally{DB.history=old}}
  function globalStatsFor(type){const all=filtered(type);let rows=DB.roster.map(p=>{let sessions=all.filter(s=>(s.present||[]).includes(p.id));if(!sessions.length)return null;let agg={sessions:sessions.length,g:0,a:0,sh:0,on:0,po:0,pb:0,r:0,l:0,dok:0,dbad:0,sv:0,points:0,votes:0};sessions.forEach(s=>{let z=pstats(p.id,s),sc=playerScore(p.id,s);agg.g+=z.g;agg.a+=z.a;agg.sh+=z.sh;agg.on+=z.on;agg.po+=z.po;agg.pb+=z.pb;agg.r+=z.r;agg.l+=z.l;agg.dok+=z.dok;agg.dbad+=z.dbad;agg.sv+=z.sv;agg.points+=sc.total;agg.votes+=sc.vote});agg.avgVote=agg.votes/agg.sessions;return{p,...agg}}).filter(Boolean).sort((a,b)=>b.avgVote-a.avgVote||b.points-a.points);if(!rows.length)return '<div class="sub">Nessun dato disponibile.</div>';return `<div class="globalstatswrap"><table class="report globaltable"><thead><tr><th>Giocatore</th><th>Ruolo</th><th>Sessioni</th><th>Voto medio</th><th>Punti</th><th>Gol</th><th>Assist</th><th>Tiri</th><th>In porta</th><th>Pass +</th><th>Pass -</th><th>Rec.</th><th>Perse</th><th>Drib +</th><th>Drib -</th><th>Parate</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${esc(x.p.name)}</td><td>${RN[x.p.role]}</td><td>${x.sessions}</td><td><b>${x.avgVote.toFixed(2)}</b></td><td>${x.points.toFixed(1)}</td><td>${x.g}</td><td>${x.a}</td><td>${x.sh}</td><td>${x.on}</td><td>${x.po}</td><td>${x.pb}</td><td>${x.r}</td><td>${x.l}</td><td>${x.dok}</td><td>${x.dbad}</td><td>${x.sv}</td></tr>`).join('')}</tbody></table></div>`}
  renderHistory=function(){const tr=filtered(TRAINING_TYPE),ga=filtered(GAME_TYPE);if($('#trainingHistoryList'))$('#trainingHistoryList').innerHTML=historyRows(tr,TRAINING_TYPE);if($('#gameHistoryList'))$('#gameHistoryList').innerHTML=historyRows(ga,GAME_TYPE);$$('[data-open]').forEach(b=>b.onclick=()=>openSession(b.dataset.open));$$('[data-rm]').forEach(b=>b.onclick=()=>{if(confirm('Eliminare definitivamente questa sessione?')){DB.history=DB.history.filter(s=>s.id!==b.dataset.rm);save();renderHistory()}});if($('#globalStats'))$('#globalStats').innerHTML=globalStatsFor(TRAINING_TYPE);if($('#gameGlobalStats'))$('#gameGlobalStats').innerHTML=globalStatsFor(GAME_TYPE);if($('#comboAnalysis'))$('#comboAnalysis').innerHTML=withHistory(TRAINING_TYPE,()=>topSevenHtml()+comboHtmlAll());if($('#gameAnalysis'))$('#gameAnalysis').innerHTML=withHistory(GAME_TYPE,()=>topSevenHtml()+comboHtmlAll())};

  exportSessionCSV=function(s){let head=['Giocatore','Ruolo','Punti','Voto','Gol','Assist','Tiri','Tiri in porta','Passaggi corretti','Passaggi sbagliati','Recuperi','Palle perse','Dribbling riusciti','Dribbling falliti','Parate'];let rows=(s.present||[]).map(pid=>{let p=DB.roster.find(x=>x.id===pid),z=pstats(pid,s),sc=playerScore(pid,s);return[p?.name||'',RN[p?.role]||'',sc.total.toFixed(1),sc.vote.toFixed(1),z.g,z.a,z.sh,z.on,z.po,z.pb,z.r,z.l,z.dok,z.dbad,z.sv]});const type=sessionType(s),ma=s.teamModules?.A||s.module||'',mb=s.teamModules?.B||s.module||'';let meta=[['SevenLab'],['Tipo',type],['Data',s.dateLabel||''],['Titolo',s.title||''],...(type===GAME_TYPE?[['Avversario',s.avversario||''],['Gol fatti',s.score?.A||0],['Gol subiti',s.score?.B||0],['Modulo iniziale',ma]]:[['Formato',`${s.size}v${s.size}`],['Modulo Squadra A',ma],['Modulo Squadra B',mb],['Risultato',`${s.score?.A||0}-${s.score?.B||0}`]]),['Durata minuti',Math.floor((s.elapsed||0)/60)],[]];let blocks=[...meta,head,...rows];if(type===GAME_TYPE){blocks.push([],['EVENTI PARTITA'],['Secondo','Minuto','Tipo','Giocatore','Team','Dettaglio']);(s.events||[]).filter(e=>!e.void).forEach(e=>{let p=DB.roster.find(x=>x.id===e.pid);blocks.push([e.t||0,Math.floor((e.t||0)/60),label(e.type),p?.name||'',e.team||'',e.type==='goal_against'?'Gol avversario':e.assistId?'Con assist':''])})}let csv=blocks.map(r=>r.map(csvCell).join(';')).join('\n');const nm=type===GAME_TYPE?`SevenLab_GAME_${safeExportName(s.avversario)}_${safeExportName(s.dateLabel)}`:`SevenLab_${safeExportName(s.dateLabel||s.title)}_${s.size}v${s.size}`;downloadText('\ufeff'+csv,nm+'.csv','text/csv;charset=utf-8');toast(type===GAME_TYPE?'CSV partita esportato':'CSV allenamento esportato')};

  openSession=function(id){let s=DB.history.find(x=>x.id===id);if(!s)return;let type=sessionType(s),ids=s.present||[],rows=ids.map(pid=>{let p=DB.roster.find(x=>x.id===pid),z=pstats(pid,s),sc=playerScore(pid,s);return`<tr><td>${esc(p?.name||'')}</td><td>${RN[p?.role]||''}</td><td>${sc.total.toFixed(1)}</td><td><b>${sc.vote.toFixed(1)}</b></td><td>${z.g}</td><td>${z.a}</td><td>${z.po}/${z.po+z.pb}</td><td>${z.dok}/${z.dok+z.dbad}</td><td>${z.r}</td><td>${z.l}</td></tr>`}).join('');let links=assistLinks(s).slice(0,5).map(x=>`<div class="combo"><b>${esc(name(x.a))} → ${esc(name(x.b))}</b><span class="meta">${x.n} assist</span></div>`).join('')||'<div class="sub">Nessuna connessione assist registrata.</div>';const result=`${s.score?.A||0}-${s.score?.B||0}`,extra=type===GAME_TYPE?`<div class="kpi"><b>${esc(s.avversario||'')}</b><small>Avversario</small></div>`:`<div class="kpi"><b>${s.size}v${s.size}</b><small>Formato</small></div>`;$('#sessionDetail').innerHTML=`<div class="kpis"><div class="kpi"><b>${result}</b><small>Risultato</small></div>${extra}<div class="kpi"><b>${Math.floor((s.elapsed||0)/60)}'</b><small>Durata</small></div></div><div class="exportbar053 single"><button class="btn primary" id="exportCsv053">📊 Esporta CSV ${type===GAME_TYPE?'partita':'allenamento'}</button></div><div style="overflow:auto"><table class="report"><thead><tr><th>Giocatore</th><th>Ruolo</th><th>Pti</th><th>Voto</th><th>G</th><th>A</th><th>Pass</th><th>Drib.</th><th>Rec.</th><th>Perse</th></tr></thead><tbody>${rows}</tbody></table></div><h3 class="mt">Connessioni gol/assist</h3>${links}`;$('#exportCsv053').onclick=()=>exportSessionCSV(s);$('#sessionModal').classList.add('show')};
